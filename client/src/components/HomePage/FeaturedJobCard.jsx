  ...
  return (
    <div className='bg-white border px-4 py-4 rounded-xl shadow-md'>
      <div className='flex items-center gap-3'>
        <img
          src={`${backendUrl}/${featuredJob.createdBy.logoUrl}`}
          alt={'Logo công ty'}
        ...
        <div className='flex-1 min-w-0'>
          <h2
            ...
            className='text-lg font-medium hover:underline hover:text-green-700 cursor-pointer line-clamp-1'
          >
            <Link target='_blank' to={`/viec-lam/${featuredJob.slug}`} className='w-full'>
              {featuredJob.title}
            </Link>
          </h2>
          <p className='text-gray-500 text-sm font-medium mt-1'>{featuredJob.createdBy.companyName}</p>
        </div>

        <Button
          title='Thêm vào việc làm yêu thích'
          onClick={handleSaveJob}
          variant='ghost'
          className={`h-8 w-8 shrink-0 border border-green-300 text-green-700 hover:bg-green-300 hover:text-white rounded-full cursor-pointer`}
        >
          <Heart />
        </Button>
      ...
      <div className=''>
        <div className='py-5'>
          <div className='flex items-center gap-2'>
            <Badge variant='ghost' className={`bg-slate-200 text-slate-900`}>
              <CircleDollarSign />
              {featuredJob.salaryMin} - {featuredJob.salaryMax}đ
            </Badge>
            <Badge variant='ghost' className={`bg-slate-200 text-slate-900`}>
              <MapPin />
              {location}
