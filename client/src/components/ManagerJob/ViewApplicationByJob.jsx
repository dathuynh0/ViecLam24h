...
const ViewApplicationByJob = ({ isOpen, onClose }) => {
    const { applications, getApplicationByJob, adminLoading } = useAdminStore();

    useEffect(() => {
        if (isOpen) {
            getApplicationByJob(isOpen?.id)
        }
    }, [isOpen])
                    ...
                    <div className="fixed flex inset-0 z-20 items-center justify-center">
                        ...
                        >
                            <h2 className="text-center text-xl font-medium p-6 pb-0">
                                Ứng viên ứng tuyển: {isOpen?.title}
                            </h2>
                                            ...
                                            applications?.map((app) => (
                                                <tr key={app.id} className="border-b hover:bg-gray-50 transition-colors">
                                                    ...
                                                    <td className="px-4 py-3 text-gray-600 max-w-[200px] truncate">
                                                        {
                                                            app?.introduction ? 
                                                                <Textarea defaultValue={app?.introduction} className={`max-h-[120px]`} />
                                                                : 'Không có'
                                                        }
                                                    </td>
...
export default ViewApplicationByJob
