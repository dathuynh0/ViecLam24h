    ...
    <div className='w-full border border-gray-300 rounded-lg p-4'>
        <div className='flex justify-between gap-4'>
            <div className='flex items-center gap-4'>
                <img src={`${import.meta.env.VITE_BACKEND_URL}/${company?.logoUrl}`} alt={`Logo nhà tuyển dụng`} className='flex h-22 w-22 shrink-0 rounded-xl p-2 shadow-sm object-contain border border-gray-400' />

                <div className='flex flex-col space-y-1'>
                    <Link to={`/viec-lam/${job?.slug}`} className='text-green-700 text-lg font-medium hover:underline'>{job?.title}</Link>
                    <p className='text-muted-foreground font-medium'>{company?.companyName}</p>
